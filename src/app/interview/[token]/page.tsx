...
export default async function CandidateInterviewPage({
  ...
  const { data: interview, error } = await supabase
    .from("interviews")
    .select("id, title, token, status, questions")
    .eq("token", token)
    .single();
        ...
        >
          <div className="w-full max-w-xl rounded-[2rem] border border-white/70 bg-white/85 p-10 text-center shadow-2xl shadow-slate-200/60 backdrop-blur">
            ...
            <p className="mt-4 text-base leading-7 text-slate-600">
              Each unique interview link can only be used for one attempt. If you had technical issues, please reach out to your recruiter for a new link.
            </p>
          </div>
        </div>
      );
